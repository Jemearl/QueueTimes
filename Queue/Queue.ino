...
void displayRideInfo(String IDNum, const char* landName, JsonObject ride) {
  ...
  if (isOpenStr == nullptr || strcmp(isOpenStr, "") == 0 || strcmp(isOpenStr, "false") == 0) {
    ...
    const char* waitTimeStr = ride["wait_time"].as<const char*>();
    if (waitTimeStr == nullptr || strlen(waitTimeStr) == 0) {
      drawOutlinedText("NA", tft.width() / 2, 130, waitTimeColor, 6);
    } else {
      drawOutlinedText(waitTimeStr, tft.width() / 2, 130, waitTimeColor, 6);
    }
    ...
    if (waitCode == 200) {
      ...
      // If there's no "lands" array, check for a flat "rides" array.
        ...
        for (JsonObject ride : waitDoc["rides"].as<JsonArray>()) {
          displayRideInfo(IDNum, "Park", ride); // Default land name value.
        }
      }
      else {
        tft.fillScreen(TFT_BLACK);
